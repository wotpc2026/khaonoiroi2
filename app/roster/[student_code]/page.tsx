...
export default function StudentProfile({ params }: { params: { student_code: string } }) {
  ...
  useEffect(() => {
    ...
    client.auth.getUser().then(async ({ data }) => {
      if (!data.user) return;
      const { data: profile } = await client.from('profiles').select('role,student_id').eq('id', data.user.id).maybeSingle();
      setRole(profile?.role === 'staff' ? 'staff' : 'member');
      setOwnStudentId(profile?.student_id || null);
    });
  }, [params.student_code]);
